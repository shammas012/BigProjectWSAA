...
from SMART_Service_Desk.app.models import TicketHistory, db, Ticket, Project, WorkflowStatus
...
def updateTicket(key):
    try:
        ...
        if changed:
            ...
            updateTicketHistoryDetails(ticket, oldStatusId)
...
def updateTicketHistoryDetails(ticket, oldStatusId, comment=None):
        historyDetails = TicketHistory(
            ticketKey=ticket.key,
            fromStatus=oldStatusId,
            toStatus=ticket.statusId,
            changedBy=get_jwt_identity(), 
            comment=comment,
            field_changed='statusId', 
            timestamp=datetime.utcnow()
        )
        db.session.add(historyDetails)
        current_app.logger.info(f"Status changed: {historyDetails.fromStatus} → {historyDetails.toStatus} by {historyDetails.changedBy}")
